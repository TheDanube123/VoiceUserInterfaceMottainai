...
class State:
    ...
    def feed(self, inp):
        for rule in self.rules:
                if rule["getfunc"] == "" or rule["getfunc"](inp) in rule["input"]:
                     if type(rule["action"]) == type(""):
                         tts.callTTS(rule["action"])
                     elif type(rule["action"] )== type(lambda x:x):
                         tts.callTTS(rule["action"](inp))
                     else:
...
if __name__ == "__main__":
    ...
    st0.setTransition(lambda x:x, ["st1",], 1, "echo goto st1".split())
    st1.setTransition(lambda x:x, ["st0",], 0, "echo goto st0".split())
